...
class GPSRTKSimulation(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GPS-Denied Multi-Vehicle RTK Simulation with Line-of-Sight Obstacles")
        ...
        self.base_stations = [
            BaseStation((10, 10), comm_range=25, noise_std=0.5, packet_loss=0.1, obstacles=[]),
            BaseStation((90, 90), comm_range=25, noise_std=0.5, packet_loss=0.1, obstacles=[]),
            BaseStation((10, 90), comm_range=25, noise_std=0.5, packet_loss=0.1, obstacles=[])
        ]
    ...
    def setup_ui(self):
        ...
        self.fig, (self.ax_main, self.ax_error) = plt.subplots(2, 1, figsize=(12, 7), height_ratios=[3, 1])
    ...
    def create_control_buttons(self, layout):
        ...
        self.start_button.setStyleSheet(button_style + "QPushButton { background-color: #4CAF50; color: white; }")
        self.start_button.clicked.connect(self.start_simulation)
        
        # Pause button
        self.pause_button = QPushButton("Pause")
        self.pause_button.setStyleSheet(button_style + "QPushButton { background-color: #FF9800; color: white; }")
        self.pause_button.clicked.connect(self.pause_simulation)
        
        # Stop button
        self.stop_button = QPushButton("Stop")
        self.stop_button.setStyleSheet(button_style + "QPushButton { background-color: #f44336; color: white; }")
        self.stop_button.clicked.connect(self.stop_application)
        
        # Reset button
        self.reset_button = QPushButton("Reset")
        self.reset_button.setStyleSheet(button_style + "QPushButton { background-color: #795548; color: white; }")
        self.reset_button.clicked.connect(self.reset_simulation)
        
        # Undo button
        self.undo_button = QPushButton("Undo")
        self.undo_button.setStyleSheet(button_style + "QPushButton { background-color: #9C27B0; color: white; }")
        self.undo_button.clicked.connect(self.undo_last_area)
        
        # Clear button
        self.clear_button = QPushButton("Clear")
        self.clear_button.setStyleSheet(button_style + "QPushButton { background-color: #607D8B; color: white; }")
        self.clear_button.clicked.connect(self.clear_areas)
        
        # Help button
        self.demo_button = QPushButton("Help")
        self.demo_button.setStyleSheet(button_style + "QPushButton { background-color: #2196F3; color: white; }")
    ...
    def create_control_panel(self, main_layout):
        ...
        self.gps_mode_radio.setStyleSheet("color: #ff6b6b; font-weight: bold; font-size: 11px;")
        self.obstacle_mode_radio.setStyleSheet("color: #95a5a6; font-weight: bold; font-size: 11px;")
        ...
        base_range_label.setStyleSheet("font-size: 11px; font-weight: 500; color: #cccccc;")
        ...
        self.base_range_label.setStyleSheet("font-size: 11px; font-weight: bold; min-width: 40px; color: #4CAF50;")
        ...
        self.noise_label.setStyleSheet("font-size: 11px; font-weight: bold; min-width: 40px; color: #FF9800;")
        param_layout.addWidget(self.noise_label, 1, 2)
        
        # Packet loss
        packet_loss_label = QLabel("Packet Loss:")
        packet_loss_label.setStyleSheet("font-size: 11px; font-weight: 500; color: #cccccc;")
        ...
        self.packet_loss_label.setStyleSheet("font-size: 11px; font-weight: bold; min-width: 40px; color: #f44336;")
        ...
        steps_label.setStyleSheet("font-size: 11px; font-weight: 500; min-width: 40px; color: #cccccc;")
        ...
        for i, rover in enumerate(self.rovers):
            ...
            header.setStyleSheet(f"font-weight: bold; color: {rover.color}; font-size: 11px; padding: 2px; background-color: #404040; border-radius: 3px;")
            ...
            for label in status_info.values():
                label.setStyleSheet("font-size: 9px; margin: 1px; padding: 1px; color: #cccccc;")
        ...
        for label in self.metrics_labels.values():
            label.setStyleSheet("font-size: 11px; margin: 2px; padding: 1px; font-weight: 500; color: #e3f2fd;")
    ...
    def trigger_replanning(self, rover):
        ...
        for base_station in self.base_stations:
            ...
            if base_station.transceiver.can_communicate(rover.true_pos, base_station.true_pos):
    ...
    def store_performance_data(self):
        ...
        for i, rover in enumerate(self.rovers):
            ...
            comm_count = sum(1 for base in self.base_stations 
        ...
        if len(self.performance_data['time']) > max_points:
            ...
            for i in range(len(self.rovers)):
                self.performance_data['rover_errors'][i] = self.performance_data['rover_errors'][i][-max_points:]
                self.performance_data['communication_status'][i] = self.performance_data['communication_status'][i][-max_points:]
                self.performance_data['waypoint_progress'][i] = self.performance_data['waypoint_progress'][i][-max_points:]
    ...
    def update_rover_status(self):
        ...
        for i, rover in enumerate(self.rovers):
            ...
            # Position
            labels['position'].setText(f"Pos: ({rover.true_pos[0]:.1f}, {rover.true_pos[1]:.1f})")
            labels['position'].setStyleSheet("font-size: 9px; margin: 1px; padding: 1px; color: #cccccc;")
            ...
            labels['waypoint'].setStyleSheet(f"color: {waypoint_color}; font-weight: bold; font-size: 9px; margin: 1px; padding: 1px;")
            ...
            labels['gps_status'].setStyleSheet(f"color: {gps_color}; font-weight: bold; font-size: 9px; margin: 1px; padding: 1px;")
            
            # Estimate
            if rover.estimate is not None:
                labels['estimate'].setText(f"Est: ({rover.estimate[0]:.1f}, {rover.estimate[1]:.1f})")
                labels['estimate'].setStyleSheet("font-size: 9px; margin: 1px; padding: 1px; color: #64B5F6;")
            else:
                labels['estimate'].setText("Est: N/A")
                labels['estimate'].setStyleSheet("font-size: 9px; margin: 1px; padding: 1px; color: #888888;")
            ...
            if rover.estimate is not None:
                ...
                if error > 5.0:
                    ...
                    error_color = "#4CAF50"
                labels['error'].setStyleSheet(f"color: {error_color}; font-weight: bold; font-size: 9px; margin: 1px; padding: 1px;")
            else:
                labels['error'].setText("Err: N/A")
                labels['error'].setStyleSheet("color: #888888; font-size: 9px; margin: 1px; padding: 1px;")
            ...
            labels['corrections'].setStyleSheet(f"color: {corrections_color}; font-size: 9px; margin: 1px; padding: 1px;")
            ...
            labels['communication'].setStyleSheet(f"color: {comm_color}; font-weight: bold; font-size: 9px; margin: 1px; padding: 1px;")
            
            # Line of sight (NEW)
            los_count = sum(1 for base in self.base_stations 
                          if base.transceiver.can_communicate(rover.true_pos, base.true_pos))
            if los_count >= 2:
                los_color = "#4CAF50"
            elif los_count == 1:
            ...
            labels['line_of_sight'].setStyleSheet(f"color: {los_color}; font-weight: bold; font-size: 9px; margin: 1px; padding: 1px;")
    ...
    def update_performance_metrics(self):
        ...
        comm_quality = (total_active / total_possible) * 100 if total_possible > 0 else 0
        self.metrics_labels['comm_quality'].setText(f"Communication Quality: {comm_quality:.1f}%")
        
        # Line-of-sight quality (NEW)
        los_active = sum(1 for rover in self.rovers
                        for base in self.base_stations
                        if base.transceiver.can_communicate(rover.true_pos, base.true_pos))
        los_quality = (los_active / total_possible) * 100 if total_possible > 0 else 0
        self.metrics_labels['line_of_sight_quality'].setText(f"Line-of-Sight Quality: {los_quality:.1f}%")
        ...
        self.metrics_labels['waypoints_completed'].setText(f"Waypoints Completed: {completed_waypoints}/{total_waypoints}")
        
        # Simulation time
        self.metrics_labels['simulation_time'].setText(f"Simulation Time: {self.simulation_time:.1f}s")
    ...
    def update_plot(self):
        ...
        self.ax_main.set_title("GPS-Denied Multi-Vehicle RTK Simulation with Line-of-Sight Obstacles", 
        ...
        for center_x, center_y, radius in self.gps_denied_areas:
            circle = plt.Circle((center_x, center_y), radius, color="red", alpha=0.3, label="GPS-Denied Area")
            self.ax_main.add_artist(circle)
        
        # Draw physical obstacles (GREY) - NEW
        for center_x, center_y, radius in self.physical_obstacles:
            circle = plt.Circle((center_x, center_y), radius, color="gray", alpha=0.6, label="Physical Obstacle")
            self.ax_main.add_artist(circle)
        
        # Draw base stations
        for base_station in self.base_stations:
            self.ax_main.scatter(*base_station.true_pos, color="purple", marker="^", s=100, 
                               edgecolor="black", linewidth=2, label="Base Station", zorder=10)
            
            # Draw communication range
            comm_circle = plt.Circle(base_station.true_pos, base_station.comm_range, 
                                   color="purple", alpha=0.1, linestyle="--", fill=False)
            self.ax_main.add_artist(comm_circle)
        
        # Draw satellites
        for sat_pos in self.satellite_positions[:8]:
            self.ax_main.scatter(sat_pos[0], sat_pos[1], color="yellow", marker="*", s=20, alpha=0.7)
        ...
        for rover in self.rovers:
            ...
            if len(rover.trajectory) > 1:
                ...
                self.ax_main.plot(trajectory_array[:, 0], trajectory_array[:, 1], 
                                color=rover.color, alpha=0.3, linewidth=1)
            
            # Draw waypoints
            if rover.waypoints:
                waypoint_array = np.array(rover.waypoints)
                self.ax_main.plot(waypoint_array[:, 0], waypoint_array[:, 1], 
                                color=rover.color, linestyle="--", alpha=0.5, linewidth=2)
                
                # Mark waypoints
                for i, wp in enumerate(rover.waypoints):
                    if i <= rover.current_waypoint_index:
                        self.ax_main.scatter(*wp, color=rover.color, marker="x", s=50, alpha=0.7)
                    else:
                        self.ax_main.scatter(*wp, color=rover.color, marker="o", s=30, alpha=0.5)
            
            # Draw current position
            marker = "o" if rover.has_gps else "s"
            self.ax_main.scatter(*rover.true_pos, color=rover.color, marker=marker, s=100, 
                               edgecolor="black", linewidth=2, zorder=5)
            
            # Draw estimate if available
            if rover.estimate is not None and not rover.has_gps:
                self.ax_main.scatter(*rover.estimate, color="lime", marker="*", s=150, 
                                   edgecolor="darkgreen", linewidth=2, zorder=4)
                
                # Draw uncertainty ellipse if covariance available
                if hasattr(rover, 'covariance') and rover.covariance is not None:
                    self.draw_uncertainty_ellipse(rover.estimate, rover.covariance[:2, :2])
                
                # Draw error line
                self.ax_main.plot([rover.true_pos[0], rover.estimate[0]], 
                                [rover.true_pos[1], rover.estimate[1]], 
                                "r--", alpha=0.5, linewidth=1)
            
            # Draw communication links with line-of-sight awareness (ENHANCED)
            for base_station in self.base_stations:
                in_range = base_station.transceiver.in_range(rover.true_pos, base_station.true_pos)
                has_los = base_station.transceiver.can_communicate(rover.true_pos, base_station.true_pos)
                
                if in_range:
                    if has_los:
                        # Active communication - GREEN solid line
                        self.ax_main.plot([rover.true_pos[0], base_station.true_pos[0]], 
                        ...
                        self.ax_main.plot([rover.true_pos[0], base_station.true_pos[0]], 
                                        [rover.true_pos[1], base_station.true_pos[1]], 
                                        "r--", alpha=0.5, linewidth=1, zorder=1)
            
            # Label
            self.ax_main.annotate(rover.id, rover.true_pos, xytext=(5, 5), 
                                textcoords="offset points", fontsize=8, 
                                color=rover.color, fontweight="bold")
        ...
        if self.performance_data['time']:
            for i, rover in enumerate(self.rovers):
                if self.performance_data['rover_errors'][i]:
                    ...
                    times = []
                    errors = []
                    for t, err in zip(self.performance_data['time'], self.performance_data['rover_errors'][i]):
                        if err != float('inf'):
                            times.append(t)
                            errors.append(err)
                    
                    if times:
                        self.ax_error.plot(times, errors, color=rover.color, 
                                         label=f"{rover.id} Error", linewidth=2)
                        has_data = True
        
        self.ax_error.set_xlabel("Time (s)", fontsize=11)
        self.ax_error.set_ylabel("Position Error (m)", fontsize=11)
        self.ax_error.set_title("Position Error (Estimate vs True Position)", fontsize=12, fontweight='bold')
    ...
    def draw_uncertainty_ellipse(self, center, covariance, n_std=2.0):
        ...
        ellipse = plt.matplotlib.patches.Ellipse(center, width, height, angle=angle, 
                                               color="lime", alpha=0.2, fill=True)
    ...
    def on_drag(self, event):
        ...
        self.drawing_state['temp_circle'] = plt.Circle(start, radius, color=color, alpha=alpha)
