...
class JammingZone:
    ...
    def __post_init__(self):
        ...
        params = OBSTACLE_PARAMS.get(
            self.obstacle_type, OBSTACLE_PARAMS[ObstacleType.LOW_JAM]
        )
    ...
    def to_v2v_spec(self):
        ...
        from swarm_squad_ep1.algo.v2v_channel import ObstacleKind, ObstacleSpec
...
class FormationState:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "roles": {
                k: v.value if isinstance(v, FormationRole) else v
...
def calculate_Jn(
    comm_matrix: np.ndarray, neighbor_matrix: np.ndarray, PT: float
) -> float:
...
def calculate_rn(
    dist_matrix: np.ndarray, neighbor_matrix: np.ndarray, PT: float
) -> float:
