...
from swarm_squad_ep1.algo.v2v_channel import reset_channel_model
...
from swarm_squad_ep1.simulation.agents import AgentState, init_agents


@asynccontextmanager
async def lifespan(app: FastAPI):
    """FastAPI lifespan hook for startup initialization."""
    await _initialize_simulation_state()
    yield


# Create FastAPI app
app = FastAPI(title="Swarm Squad Ep1 Simulation API", lifespan=lifespan)
...
async def _initialize_simulation_state():
    ...
    for i, obs in enumerate(initial_obstacles):
        ...
        print(
            f"[SIM] Loaded {obstacle_type.value} obstacle {zone_id}: center=({obs[0]}, {obs[1]}, {obs[2] if len(obs) > 2 else 0}), radius={obs[3] if len(obs) > 3 else 5.0}"
        )
    ...
    for i, sz in enumerate(initial_spoof_zones):
        ...
        print(
            f"[SIM] Loaded {spoof_type.value} spoofing zone {zone_id}: center=({sz[0]}, {sz[1]}, {sz[2] if len(sz) > 2 else 0}), radius={sz[3] if len(sz) > 3 else 10.0}"
        )
    ...
    print(
        f"[SIM] LLM Assistance: {'enabled' if llm_assistance_enabled else 'disabled'}"
    )
...
async def create_agent(data: dict[str, Any]):
    ...
    if not (X_RANGE[0] <= x <= X_RANGE[1]):
        raise HTTPException(
            status_code=400, detail=f"X coordinate {x} outside boundaries {X_RANGE}"
        )
    if not (Y_RANGE[0] <= y <= Y_RANGE[1]):
        raise HTTPException(
            status_code=400, detail=f"Y coordinate {y} outside boundaries {Y_RANGE}"
        )
    if not (Z_RANGE[0] <= z <= Z_RANGE[1]):
        raise HTTPException(
            status_code=400, detail=f"Z coordinate {z} outside boundaries {Z_RANGE}"
        )
...
async def delete_agent(agent_id: str):
    ...
    return {"success": True, "message": f"Deleted {agent_id}"}
...
async def simulate_step():
    ...
    """
    from swarm_squad_ep1.simulation.agents import move_agent_towards_target
...
async def create_jamming_zone(zone_data: dict[str, Any]):
    ...
    if type_str == "none":
        return {
            "success": False,
            "message": "Obstacle type 'none' selected -- no zone created",
        }
    try:
        obstacle_type = ObstacleType(type_str)
    except ValueError:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid obstacle_type '{type_str}'. Must be 'physical', 'low_jam', or 'high_jam'",
        )
    ...
    print(
        f"[SIM API] Created {obstacle_type.value} zone {zone_id} at {center} with radius {radius}"
    )
...
async def update_jamming_zone(zone_id: str, zone_data: dict[str, Any]):
    ...
    if "obstacle_type" in zone_data:
        ...
        try:
            ...
            params = OBSTACLE_PARAMS.get(
                new_type, OBSTACLE_PARAMS[ObstacleType.LOW_JAM]
            )
...
async def create_spoofing_zone(zone_data: dict[str, Any]):
    ...
    zone = SpoofingZone(
        ...
        coordinate_vector=[
            float(v) for v in zone_data.get("coordinate_vector", [10.0, 10.0, 0.0])
        ],
    )
    spoofing_zones[zone_id] = zone
    print(
        f"[SIM API] Created {spoof_type.value} spoofing zone {zone_id} at {center} r={radius}"
    )
...
async def get_v2v_channel_state():
    ...
    from swarm_squad_ep1.algo.v2v_channel import get_channel_model
...
async def reset_simulation():
    ...
    print(
        f"[SIM API] Simulation reset (reloaded {len(jamming_zones)} jamming + {len(spoofing_zones)} spoofing zones)"
    )
...
async def get_visualization_data(trail_length: str = "short"):
    ...
    vis_data["llm_targets"] = {aid: target for aid, target in llm_targets.items()}
    ...
    bus_phantoms = {
        pid: pos
        for pid, pos in bus.get_perceived_positions([]).items()
    ...
    vis_data["phantom_agents"] = (
        bus_phantoms
        or get_spoofing_engine().get_phantom_positions(list(spoofing_zones.values()))
    )
    ...
    while simulation_running:
        ...
        if bus is not None:
            ...
            if spoof_list and spoof_engine:
                ...
                spoofed_messages = spoof_engine.process(
                    bus.get_messages(), spoof_list, agent_pos
                )
                bus.set_messages(spoofed_messages)

            # 3. Packet loss (tied to existing comm quality / jamming)
            comm_quals = {
                aid: float(a.communication_quality) for aid, a in agent_states.items()
            }
        ...
        if llm_assistance_enabled:
            ...
            discovered_obs = (
                controller.get_discovered_obstacles()
                ...
                if guidance and agent_id in commands:
                    ...
                    if cmd.target_position:
                        ...
                        if control_magnitude > max_llm_control:
                            blended_control = blended_control * (
                                max_llm_control / control_magnitude
                            )
                        ...
                        new_target = np.clip(
                            new_target, controller.bounds_min, controller.bounds_max
                        )
                        cmd.target_position = new_target.tolist()

                        print(
                            f"[LLM] Applied guidance to {agent_id}: {guidance.reasoning[:50]}..."
                        )
        ...
        for agent_id, cmd in commands.items():
            if agent_id in agent_states and cmd.target_position:
                ...
                if agent_id in llm_targets:
                    ...
                    new_pos = move_agent_towards_target(
                        agent, user_target, max_step=1.0
                    )
                    ...
                    dist = (
                        sum((a - b) ** 2 for a, b in zip(new_pos, user_target)) ** 0.5
                    )
                    if dist < 1.0:  # Reached target
                        print(
                            f"[SIM] {agent_id} reached user-commanded target {user_target}"
                        )
                        print(
                            f"[SIM] {agent_id} resuming formation control and path planning"
                        )
                        agent.llm_target = None
                        del llm_targets[agent_id]
        ...
        avg_comm = sum(a.communication_quality for a in agent_states.values()) / max(
            1, len(agent_states)
        )
...
def _finalize_simulation_results():
    ...
    if simulation_results["Jn_history"]:
        simulation_results["avg_Jn"] = sum(simulation_results["Jn_history"]) / len(
            simulation_results["Jn_history"]
        )
        simulation_results["final_Jn"] = simulation_results["Jn_history"][-1]

    if simulation_results["rn_history"]:
        simulation_results["avg_rn"] = sum(simulation_results["rn_history"]) / len(
            simulation_results["rn_history"]
        )
    ...
    if hasattr(controller, "_agent_paths"):
        ...
        if num_agents > 0:
            simulation_results["avg_traveled_path"] = round(
                total_path_length / num_agents, 2
            )
            simulation_results["per_agent_path_lengths"] = path_lengths
            print(
                f"[SIM API] Path lengths: avg={simulation_results['avg_traveled_path']:.2f}, per-agent={path_lengths}"
            )
    ...
    print(
        f"[SIM API] Results finalized: {simulation_results['steps']} steps, {simulation_results['duration_seconds']:.1f}s"
    )
...
async def download_simulation_results(format: str = "json"):
    ...
    if format == "csv":
        ...
        for i, ts in enumerate(simulation_results.get("timestamps", [])):
            Jn = (
                simulation_results["Jn_history"][i]
                if i < len(simulation_results["Jn_history"])
                else 0
            )
            rn = (
                simulation_results["rn_history"][i]
                if i < len(simulation_results["rn_history"])
                else 0
            )
            jammed = (
                simulation_results["jammed_count_history"][i]
                if i < len(simulation_results["jammed_count_history"])
                else 0
            )
            comm = (
                simulation_results["avg_comm_quality_history"][i]
                if i < len(simulation_results["avg_comm_quality_history"])
                else 0
            )
...
if __name__ == "__main__":
    ...
    from swarm_squad_ep1.config import SIM_API_PORT
